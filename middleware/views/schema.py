...
class Query(graphene.ObjectType):
    wosbyyear = List(WOS, year=Int(required=True))
    wosbyid = List(WOS, wosid=String(required=True))

    def resolve_wosbyyear(self, info, year):
        try:
    ...
    def resolve_wosbyid(self, info, wosid):
        try:
            connection = connection_pool.getconn()
            cursor = connection.cursor()
            auth_token = info.context.environ['HTTP_AUTH_TOKEN']
            ...
            if status_code == 200:
                ...
                if role_found:
                    logger.info('User has wos role')
                    # call stored procedure
                    cursor.callproc('show_wos_summary_bywosid', [wosid, ])
                    result = cursor.fetchall()
                    # Convert query results to objects of key-value pairs
                    objects_list = []
                    for row in result:
